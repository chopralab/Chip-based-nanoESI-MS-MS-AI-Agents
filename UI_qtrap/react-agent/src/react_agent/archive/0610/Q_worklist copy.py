...
# 1) Main worklist input directory (user supplies filename)
INPUT_WORKLIST_DIR = os.path.join(SCRIPT_DIR, "data", "worklist", "input")
...
class WorklistGenerator:
    def __init__(
        self,
        input_filename: str,   # just the filename, not full path!
        methods_csv: str,
        output_dir: str,
    ):
    ...
    def generate_worklist(self) -> str:
        # Build full input path from filename
        worklist_csv = os.path.join(INPUT_WORKLIST_DIR, self.input_filename)
        df_input = pd.read_csv(worklist_csv)
        ...
        # Remove extension for the output base
        input_base = os.path.splitext(self.input_filename)[0]
        time_str = f"_{hour}{minute}{ampm}"
        out_name = f"{input_base}_{today}{time_str}.csv"
        ...
        for _, row in df_input.iterrows():
            ...
            for method in matched_methods:
                # Optionally, append method name to sample for uniqueness if multiple
...
class WorklistState(TypedDict):
    ...
    input_filename: str  # <-- added, must be supplied by user
...
async def generate_worklist_node(
    ...
    try:
        ...
        if not input_filename:
            msg = "No input filename specified."
            logger.error(msg)
            return {
                **state,
        ...
        path = await asyncio.to_thread(
            WorklistGenerator(input_filename, METHODS_CSV, OUTPUT_DIR).generate_worklist
        )
