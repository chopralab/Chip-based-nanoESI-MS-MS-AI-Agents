...
from Q_convert import move_wiff_pairs

# ----------------------------------------------------------------------------
# Date Configuration
# ----------------------------------------------------------------------------
def get_date_str(date_arg=None):
    if date_arg:
        return date_arg
    return datetime.now().strftime("%Y%m%d")

# Directory configuration functions

def get_qc_text_target_dir(date_str):
    base = Path("/home/qtrap/sciborg_dev/UI_qtrap/react-agent/src/react_agent/data/qc/text")
    return base / date_str

def get_qc_csv_target_dir(date_str):
    base = Path("/home/qtrap/sciborg_dev/UI_qtrap/react-agent/src/react_agent/data/qc/csv")
    return base / date_str

def get_log_dir(date_str):
    base = Path("/home/qtrap/sciborg_dev/UI_qtrap/react-agent/src/react_agent/data/logs/qc")
    d = base / date_str
    d.mkdir(parents=True, exist_ok=True)
    return d

def get_log_file(date_str):
    return get_log_dir(date_str) / f"QC_{date_str}.log"
...
# File handler will be set up in main logic once date_str is known
file_handler = None
...
WSL_TARGET_DIR = get_qc_text_target_dir(get_date_str())
...
# ----------------------------------------------------------------------------
def qc_results(date_str: str = get_date_str()) -> None:
    """
    ...
    source_dir = get_qc_csv_target_dir(date_str)
...
def qc_validated_move(date_str: str = get_date_str()) -> None:
    """
...
async def convert_and_parse_node(state: QCState, config: RunnableConfig, date_str=None) -> QCState:
    # Use date_str from argument or state, fallback to today
    date_str = date_str or state.get('date') or get_date_str()
    global file_handler
    # Set up file handler for logging with correct date
    if file_handler is None:
        file_handler = logging.FileHandler(get_log_file(date_str), mode='w')
        file_handler.setLevel(logging.DEBUG)
        file_formatter = logging.Formatter('%(asctime)s [%(levelname)s] %(name)s: %(message)s')
        file_handler.setFormatter(file_formatter)
        logger.addHandler(file_handler)

    # 1. Run conversion (move_wiff_pairs from Q_convert)
    moved, debug = await move_wiff_pairs(date_str=date_str)
    msg1 = f"✅ Conversion step done. Files moved: {moved}"
    logger.info(msg1)
    logger.debug("\n".join(debug))

    # 2. Prepare parsing
    qc_csv_target_dir = get_qc_csv_target_dir(date_str)
    qc_csv_target_dir.mkdir(parents=True, exist_ok=True)
    qc_text_target_dir = get_qc_text_target_dir(date_str)
    files = list(qc_text_target_dir.glob("*.txt"))
    results: List[str] = []
    for src in files:
        base = src.stem
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="QC Convert and Parse Pipeline")
    ...
    # Set up logging file handler for this date
    if file_handler is None:
        file_handler = logging.FileHandler(get_log_file(date_str), mode='w')
        file_handler.setLevel(logging.DEBUG)
        file_formatter = logging.Formatter('%(asctime)s [%(levelname)s] %(name)s: %(message)s')
    ...
    # Run the convert and QC pipeline
    state = {'messages': [], 'converted_files': None, 'parsing_result': None, 'agent_state': {}, 'date': date_str}
    result = asyncio.run(convert_and_parse_node(state, None, date_str=date_str))
