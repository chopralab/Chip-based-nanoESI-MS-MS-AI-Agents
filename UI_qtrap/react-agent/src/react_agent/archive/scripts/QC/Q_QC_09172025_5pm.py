...
async def convert_and_parse_node(state: QCState, config: RunnableConfig) -> QCState:
    ...
    # Step 0: Run diagnostics to check file flow
    ...
    try:
        ...
        for src in files:
            ...
            # Check if filename date matches processing date
            filename_date_match = re.search(r'^(\d{8})_', base)
            if filename_date_match:
                filename_date = filename_date_match.group(1)
                if filename_date != date_str:
                    logger.warning(f"⚠️ Date mismatch: Processing date {date_str} but file date {filename_date} in {src.name}")
