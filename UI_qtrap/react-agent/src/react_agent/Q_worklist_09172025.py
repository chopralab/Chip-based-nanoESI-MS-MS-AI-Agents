...
class WorklistGenerator:
    ...
    def generate_worklist(self) -> str:
        ...
        for col in ['Date', 'Technical_Replicate', 'Operator']:
        ...
        needed_cols = ['Info', 'Lipid', 'Date', 'Technical_Replicate', 'Operator']
        ...
        for _, row in df_input.iterrows():
            info = row['Info'].strip()
            lipid = row['Lipid'].strip()
            date = str(row['Date']).replace("-", "")
            oper = row['Operator'].strip()
            ...
            if not methods:
                for rep in range(1, num_reps+1):
                    sample_name = f"{date}_{info}_{lipid}_{rep}_{oper}"
                ...
                for method in methods:
                    for rep in range(1, num_reps+1):
                        sample_name = f"{date}_{info}_{lipid}_{rep}_{oper}_{method}"
                        rec = {col: self.default_values.get(col, "") for col in self.column_headers}
                        rec["SampleName"] = sample_name
