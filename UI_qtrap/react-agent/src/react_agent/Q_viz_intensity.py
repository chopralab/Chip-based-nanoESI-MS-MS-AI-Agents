...
def prepare_visualization_data(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    # Sort by BaseId for consistent plot order
    agg_data = agg_data.sort_values('BaseId').reset_index(drop=True)
...
def create_highest_tic_plot(df: pd.DataFrame, output_dir: str, project_name: str, lipid_class: str):
    ...
    sns.barplot(data=df, x='BaseId', y='Highest_Summed_TIC', color='skyblue', ax=ax)
    
    ax.set_title(f'Highest Summed TIC for {lipid_class} in Project: {project_name}', fontsize=16, weight='bold')
    ax.set_xlabel('Base ID', fontsize=12)
...
def create_average_tic_plot(df: pd.DataFrame, output_dir: str, project_name: str, lipid_class: str):
    ...
    sns.barplot(data=df, x='BaseId', y='Average_Summed_TIC', color='lightgreen', ax=ax)
    
    ax.set_title(f'Average Summed TIC for {lipid_class} in Project: {project_name}', fontsize=16, weight='bold')
    ax.set_xlabel('Base ID', fontsize=12)
...
def generate_intensity_win_summary(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Count wins for each BaseId
    highest_wins = highest_tic_winners['BaseId'].value_counts().reset_index()
    highest_wins.columns = ['BaseId', 'Highest_TIC_Wins']
    
    average_wins = average_tic_winners['BaseId'].value_counts().reset_index()
    average_wins.columns = ['BaseId', 'Average_TIC_Wins']

    # Merge the two summaries
    summary_df = pd.merge(highest_wins, average_wins, on='BaseId', how='outer').fillna(0)
...
def generate_detailed_win_report(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Find winner for Highest TIC for each lipid class
    highest_tic_winners = df.loc[df.groupby('LipidClass')['Highest_Summed_TIC'].idxmax()]
    highest_tic_winners = highest_tic_winners[['LipidClass', 'BaseId', 'Highest_Summed_TIC']].rename(columns={
        'BaseId': 'Highest_TIC_Winner_BaseId',
        'Highest_Summed_TIC': 'Highest_TIC_Value'
    })

    # Find winner for Average TIC for each lipid class
    average_tic_winners = df.loc[df.groupby('LipidClass')['Average_Summed_TIC'].idxmax()]
    average_tic_winners = average_tic_winners[['LipidClass', 'BaseId', 'Average_Summed_TIC']].rename(columns={
        'BaseId': 'Average_TIC_Winner_BaseId',
