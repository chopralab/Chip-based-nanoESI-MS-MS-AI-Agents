...
async def move_wiff_pairs(
    ...
    # When processing a specific project, process ALL files regardless of date
    if project_name:
        debug_log.append(f"📅 Processing ALL files for project '{project_name}' (ignoring modification date)")
        matching_files = []
        ...
        # When no project specified, use original date-based filtering
    ...
    debug_log.append(f"📊 Total files found for processing: {len(matching_files)}")
    ...
    if not pairs_to_copy:
        if project_name:
            debug_log.append(f"❌ No complete file pairs found for project '{project_name}'")
        else:
